...
At the moment, two genomes are available. hxb2 is the one loaded by default but
it can be changed for mac239 using the genome argument.
<<loadFeatures-genome, eval=T, echo=TRUE, results=verbatim>>=
SIV_env<-loadFeatures(genome="mac239")
@
...
or by providing database environment and a feature and hxb2 coornidates range to \Rfunction{getAA} method:
